...
> 1. [Bagian 8 — Status Review & Prioritas Perbaikan](#8-status-review--prioritas-perbaikan) — 3 isu keamanan yang HARUS diperbaiki duluan.
> 2. [Bagian 28 — Checklist Final](#28-checklist-final-sebelum-submitdeploy) — daftar centang sebelum submit lomba/deploy.
...
2. Welcome carousel (5 slide)
3. Sign In / Sign Up (email, password, Google/social, 2FA)
4. Forgot Password
...
1. Select Avatar, Profile Setup (nama, bio, dll.)
2. Password Setup (strength meter) — *kalau belum diisi saat signup di `/onboarding`*
3. OTP Setup & Verifikasi (4 digit)
4. Fingerprint Setup (opsional, skip kalau device tidak support)
5. Notification Setup (toggle companionNotif/wellnessNotif/communityNotif — langsung terhubung ke model `NotificationPref`, Bagian 17)
6. **Kuisioner kesehatan mental**: goal, gender, usia, berat, mood, riwayat bantuan profesional, gejala fisik, kualitas tidur, level stres, obat, gejala mental lain, **AI Expression Analysis**
7. Compiling Data (loading state)
8. "You're All Set Up" — di titik inilah `User.onboardingCompleted` di-set `true`
...
5. **Modal vs full page**: Delete Conversation, OTP Verification, Forgot Password → jadi **modal/dialog** di desktop.
6. **Komponen reusable**: mood selector & progress ring dipertahankan ukurannya, disusun ulang dalam grid yang lebih lega.
7. **Breakpoint**: desktop ≥1024px = sidebar+grid; tablet 768–1023px = sidebar collapse icon-only; <768px = pola mobile asli.
...
**Lokasi**: `src/app/api/auth/route.ts` — `passwordHash: password || "demo_password"`.
**Masalah**: kalau database bocor, semua password pengguna (dan data assessment kesehatan mental yang terhubung ke akun mereka) langsung terbuka.
**Perbaikan**: hash password dengan `bcrypt` atau `argon2` sebelum disimpan, dan `compare()` saat login — jangan pernah simpan/bandingkan password mentah.
...
**Lokasi**: `src/app/api/auth/route.ts` — `sessionToken = \`demo_${user.id}\``.
**Masalah**: siapa pun yang tahu/menebak `user.id` (angka/string berurutan atau bisa di-enumerasi) bisa memalsukan cookie `auth-token` dan login sebagai user lain tanpa password.
**Perbaikan**: gunakan token random & ditandatangani server (JWT dengan secret, atau UUID session yang disimpan di tabel `Session` terpisah dan divalidasi tiap request), bukan string yang bisa dikonstruksi dari data publik.

### 8.3 Kode OTP dikembalikan di response API (PRIORITAS TINGGI)
**Lokasi**: `src/app/api/auth/route.ts` — field `demoCode` dikirim balik ke client.
**Masalah**: OTP yang seharusnya jadi bukti kepemilikan email, kalau dikembalikan langsung di response, sama saja tidak ada verifikasi — siapa pun bisa signup atas nama email siapa pun tanpa akses ke email itu.
**Perbaikan**: kirim OTP **hanya** lewat email/SMS asli (integrasi email service), jangan pernah sertakan di body response, bahkan untuk mode "demo" — ganti demo mode dengan OTP tetap (mis. `000000`) yang didokumentasikan di README internal tim, bukan dikirim dinamis di response.

### 8.4 Landing page publik belum ada (PRIORITAS SEDANG)
**Lokasi**: `src/app/page.tsx` — saat ini langsung me-render `DashboardPage`.
**Masalah**: siapa pun yang membuka domain root langsung melihat data dashboard tanpa login.
**Perbaikan**: ganti dengan landing page publik. Spesifikasi & kode lengkap ada di **Bagian 9**.
...
**Perbaikan**:
1. Tambahkan `data/*.json` ke `.gitignore`, hapus dari git history (`git rm --cached` lalu commit).
2. Dokumentasikan mekanisme fallback ini secara eksplisit di README (saat ini tidak disebut sama sekali) — termasuk peringatan bahwa data fallback tidak persistent di production.
3. Pertimbangkan ganti strategi fallback: kalau tujuannya cuma untuk demo tetap jalan saat Neon cold-start, gunakan in-memory cache dengan retry, bukan file lokal yang ter-commit.

### 8.5.3 Komponen bernama "Discord" (`src/components/discord/`)
`DiscordProfilePopover.tsx` dan `DiscordSettingsModal.tsx` — diimpor langsung ke `Sidebar.tsx` utama — meniru sistem presence Discord (status online/idle/dnd/invisible + custom status teks). Ini masalah yang sama kategorinya dengan isu "Doctor Freud.AI" di Bagian 2: nama file & konsep UI menempel ke brand/produk lain, bukan ZYBA.
**Perbaikan**:
...
1. **Nav bar**: logo ZYBA kiri, menu tengah (Fitur/Tentang/Komunitas) hidden di mobile, tombol "Masuk/Daftar" kanan → `/onboarding`.
2. **Hero**: badge kecil "GEN Z WELLNESS SUPPORT" (bg `orange-100`, teks `orange-500`), headline besar (`font-display`, 5xl–6xl) dengan kata kunci "Fisik" di-highlight `text-green-500` dan "Sosial" di `text-orange-500`, subteks 1 kalimat, dua CTA ("Mulai Sekarang →" solid orange, "Lihat Fitur" outline).
3. **Stat row**: 4 angka besar (mis. dari Market Size pitch deck: 74,9jt Gen Z Indonesia, 81,1% pernah curhat ke AI, target 100rb+ pengguna, 3 aspek Mental/Fisik/Sosial).
4. **Grid fitur** (4 kartu, 1 baris di desktop): Zyba Companion, Mood Check-In, Smart Activity Planner, Zyba Community — tiap kartu: emoji/ikon, judul, deskripsi singkat.
5. **CTA footer**: section dengan bg `green-100`, headline ajakan, tombol "Gabung Gratis →".
...
1. Header: "Batal" (teks link, kiri) — judul "Postingan Baru" (tengah/kiri, bold) — ikon "..." opsi lain (kanan).
2. Baris identitas: avatar bulat kecil user + nama + (opsional) breadcrumb "Komunitas atau topik" kalau fitur topik/kategori sudah ada, kalau belum jangan dipaksakan.
3. Textarea placeholder "Ada cerita apa hari ini?"
4. Baris ikon attachment (outline, `text-brown-700`, ukuran kecil sejajar horizontal): gambar, GIF, emoji, stiker, polling, dokumen, lokasi, musik — sesuaikan mana yang relevan/sudah ada backend-nya, jangan tambah ikon untuk fitur yang belum diimplementasikan.
5. Footer: kiri "Opsi Postingan" (teks kecil, opsional), kanan tombol "Post" (`bg-orange-500`, teks putih, rounded-pill).
...
1. `CreatePostModal` diimpor dan dirender di `community/page.tsx`, dengan state `isOpen`/`showModal` yang benar-benar mengontrol tampil/tidaknya.
2. Tombol "+" mengambang → `onClick` mengubah state itu jadi `true` (bukan lagi `onClick={handleScrollToCompose}`).
3. Style di dalam `CreatePostModal.tsx` diganti dari `bg-black/40 backdrop-blur-sm flex items-center justify-center` (centered dark modal) menjadi floating card kanan-bawah sesuai spek di atas.
4. Test manual: klik tombol "+" di browser (bukan cuma baca kode), pastikan card benar-benar muncul di layar.
...
1. **Spacing/margin khusus mobile yang lupa di-reset di desktop.** Kalau ada `pt-16` atau sejenisnya di `<main>`/wrapper konten untuk menghindari tombol hamburger `fixed` menutupi konten di mobile, pastikan itu ditulis `pt-16 md:pt-0` (atau breakpoint yang sesuai) — **bukan** `pt-16` polos yang kebawa ke semua ukuran layar.
2. **Wrapper sidebar `fixed md:sticky ... h-screen`** — cek apakah ada elemen sibling di `<div class="flex">` (level yang sama dengan spacer `w-64` dan `<main>`) yang ikut memengaruhi tinggi/posisi vertikal parent flex container-nya karena perubahan structural saat menambahkan drawer mobile.
3. **`align-items` pada parent flex container** (`<div class="flex">` yang membungkus spacer sidebar + `<main>`) — kalau berubah dari default `stretch` jadi `center` (sengaja atau tidak sengaja saat refactor), `<main>` akan menyusut ke tinggi kontennya sendiri dan ke-center vertikal di tengah tinggi sidebar yang `h-screen`, menyisakan ruang kosong di atas.
...
1. Buat 3 database di Neon (dalam 1 project atau 3 project terpisah — keputusan biaya).
2. Pecah `prisma/schema.prisma` yang lama jadi 3 file sesuai struktur di 17.2.
3. Ganti semua import `backend/db/prisma.ts` di seluruh route API (`api/mood`, `api/auth`, `api/community`, `api/companion`, dll.) ke client yang sesuai domainnya (17.4).
4. Karena app masih tahap demo dan belum ada data user asli yang signifikan (lihat Bagian 14 — audit dummy data), **aman untuk push schema baru dari nol ke 3 DB baru** tanpa migrasi data lama. Kalau ternyata sudah ada data user asli yang penting, itu harus di-export dulu manual sebelum DB lama di-drop.
5. Sekalian nonaktifkan/hapus `data/*.json` fallback storage (Bagian 8.5.2) di tahap ini — jangan sampai ada fallback lama yang campur aduk dengan 3 DB baru dan bikin data jadi tidak konsisten sumbernya.
6. Update `.env.example` dan `README.md` dengan 6 environment variable baru (17.3), hapus `DATABASE_URL`/`DIRECT_URL` lama.
...
1. Hapus entry `"claude-3-5-sonnet"` dan `"gpt-4o"` dari `MODEL_OPTIONS`.
2. Tambah entry untuk Mistral (mis. `"mistral-small"`) dan OpenRouter (mis. salah satu model `:free` yang tersedia).
3. Ganti default dari `gemini-1.5-flash` kalau nama model itu sudah tidak akurat — cek nama model Gemini Flash yang aktif saat ini di free tier (nama model berubah dari waktu ke waktu, jangan hardcode nama versi lama).
4. Sambungkan `onSelect`/`onChange` di komponen ini supaya benar-benar mengubah provider yang dipanggil saat `handleSendMessage` jalan — bukan cuma mengubah label yang ditampilkan.
...
1. User pilih file (gambar/rekam audio) di client — attach `onClick` ke tombol 📎 yang membuka `<input type="file" accept="image/*,audio/*" />` tersembunyi, atau trigger UI upload sejenis.
2. Client upload langsung ke Vercel Blob lewat client-upload token (bukan lewat server dulu, biar server tidak jadi bottleneck untuk file besar) — ikuti pola `@vercel/blob` client upload.
3. Blob URL yang didapat dikirim ke API route (`/api/companion/message` atau `/api/community/post`) sebagai `attachmentUrl`, disimpan ke DB.
4. Kalau attachment-nya audio dan dikirim ke Companion: server transkrip dulu via Groq Whisper, jalankan `detectRisk()` ke hasil transkrip, baru proses seperti pesan teks biasa.
...
1. **Hapus** `CompanionProjectsView.tsx`, `CompanionArtifactsView.tsx`, `CompanionCodeView.tsx`, dan seluruh routing `/companion/projects`, `/companion/artifacts`, `/companion/code`, `/companion/customize` — cek dulu dengan `grep -r` sebelum hapus, pastikan tidak ada bagian lain yang bergantung ke ini.
2. **Repurpose** `[slug]` yang sudah ada (setelah dibersihkan dari poin 1) untuk fungsi yang benar: `/companion/[slug]` sebagai route per-percakapan, di mana `slug` = `conversationId` (atau slug singkat yang di-generate dari `conversationId`).
3. Klik item di `CompanionSidebar` → ganti dari update state langsung menjadi `<Link href={`/companion/${conversation.id}`}>` atau `router.push()`, supaya `activeConvId` di `CompanionContext` di-derive dari URL param (`useParams()`), bukan `useState` yang berdiri sendiri.
4. Pastikan `conversations` di `CompanionContext` benar-benar di-fetch dari Companion DB (Bagian 17) via API, bukan array hardcoded dengan `"conv-1"` sebagai default — ini bagian dari audit dummy data di Bagian 14.
5. `companion/page.tsx` (root, tanpa slug) tetap jadi halaman default — redirect ke percakapan terbaru kalau ada, atau tampilkan empty state (Bagian 10.3) kalau user belum punya percakapan sama sekali.
...
1. **Bagian 17 (Multi-DB)** dulu — semua bagian lain butuh struktur database yang benar sebagai fondasi.
2. **Bagian 19 (Multi-provider AI)** — supaya Companion punya otak yang beneran jalan, bukan dummy response.
3. **Bagian 20 (Media attachment)** — nempel ke Companion/Community yang sudah punya AI + DB yang benar.
4. **Bagian 21 (Persona hewan)** — nempel ke provider AI yang sudah jalan (system prompt beda per persona).
5. **Bagian 22-24 (konsistensi warna, slug routing, settings/privasi)** — polish akhir setelah fondasi data & AI-nya solid.
6. **Bagian 13-16 (performa, hapus dummy, responsif, kebersihan repo)** — audit menyeluruh di tahap paling akhir, setelah semua fitur baru di atas selesai (audit dummy data harus dilakukan **setelah** semua koneksi backend nyata terpasang, supaya tidak ada yang keliru dianggap "sudah real" padahal masih ada sisa dummy dari proses development).
...
1. **Halaman "Zyba Plus"** (`/settings/zyba-plus` atau tab tersendiri, sesuai nav yang sudah ada di Bagian 24) — dua kartu berdampingan: **Zyba Free** (paket saat ini, kalau user masih free) vs **Zyba Plus** dengan daftar fitur eksklusif (sesuai Business Model pitch deck Bagian 16: penggunaan AI lebih banyak, rekomendasi lebih personal, program komunitas khusus, insight kebiasaan, laporan perkembangan) + harga per bulan.
2. User klik **"Upgrade ke Zyba Plus →"** pada kartu Plus.
3. Client memanggil `POST /api/billing/checkout` → server membuat record `Subscription` (status `PENDING`) + `Payment` (status `PENDING`, `orderId` unik), lalu memanggil **Midtrans Snap API** untuk mendapatkan `snapToken`.
4. Client menerima `snapToken`, panggil `window.snap.pay(snapToken, { onSuccess, onPending, onError, onClose })` — Midtrans menampilkan **popup pembayaran bawaan mereka** (pilih metode: transfer bank/e-wallet/kartu), **ZYBA tidak perlu desain layar ini sendiri**.
5. Setelah user selesai bayar di popup Midtrans:
...
6. Endpoint webhook **wajib** verifikasi signature (Midtrans mengirim `signature_key` yang dihitung dari `order_id + status_code + gross_amount + ServerKey`, dicocokkan di server) — **tolak** request yang signature-nya tidak valid, supaya tidak ada yang bisa memalsukan "pembayaran sukses".
7. Setelah signature valid dan status pembayaran `settlement`/`capture` (sukses menurut Midtrans): update `Payment.status = SUCCESS`, `Subscription.status = ACTIVE`, set `startDate`/`endDate` (+30 hari dari sekarang), dan update `User.plan = PLUS`.
8. Halaman konfirmasi (`/settings/zyba-plus/success`) menampilkan ringkasan: paket aktif, tanggal mulai, tanggal berakhir, tombol kembali ke Dashboard. Sidebar/badge di seluruh app otomatis berubah dari "Zyba Free" jadi "Zyba Plus PRO" (Bagian 22 — konsistensi identitas).
9. **Riwayat Pembayaran** (`/settings/billing`, nav item sudah ada) — list semua `Payment` milik user: tanggal, jumlah, status, metode. Ini juga tempat user lihat kalau pembayaran gagal/pending, bukan cuma yang sukses.
...
```mermaid
flowchart TD
    Landing["/  (Landing Page)"] -->|Mulai Sekarang| Onboarding

    subgraph Auth["Auth & Onboarding"]
        Onboarding["/onboarding<br/>(Welcome + Sign Up/In + OTP)"]
        Login["/login"]
        Assessment["/assessment<br/>(Gate wajib sebelum akses app)"]
        Welcome["/welcome<br/>(Tour setelah signup)"]
        Onboarding --> Assessment
        Login --> Dashboard
        Assessment --> Welcome --> Dashboard
    end

    subgraph MainApp["Main App (sidebar utama, butuh onboardingCompleted)"]
        Dashboard["/dashboard (Home)"]
        Mood["/mood-check-in"]
        Companion["/companion"]
        CompanionSlug["/companion/[slug]<br/>(per-percakapan)"]
        Activity["/activity<br/>(Smart Activity Planner)"]
        Wellness["/wellness-journey"]
        Community["/community (Feed)"]
        CommunitySlug["/community/[slug]<br/>(search, messages, activity, profile, insights)"]
        Resources["/resources"]
        Settings["/settings"]

        Dashboard --- Mood
        Dashboard --- Companion
        Companion -->|contextual sidebar| CompanionSlug
        Dashboard --- Activity
        Dashboard --- Wellness
        Dashboard --- Community
        Community -->|contextual sidebar| CommunitySlug
        Dashboard --- Resources
        Dashboard --- Settings
    end

    subgraph SettingsSub["Sub-halaman Settings"]
        SetProfil["Info Profil"]
        SetPassword["Password & Keamanan"]
        SetPrivacy["Data & Privasi"]
        SetDelete["Hapus Akun"]
        SetPersona["Karakter Zyba"]
        SetNotif["Notifikasi"]
        SetVisibility["Visibilitas Postingan"]
        SetPlus["/settings/zyba-plus"]
        SetPlusSuccess["/settings/zyba-plus/success"]
        ...
        Settings --> SetPlus --> SetPlusSuccess
        Settings --> SetBilling
    end
```
...
**Perbaikan**:
1. Pilih **satu** package manager — karena sudah eksplisit ditambahkan pnpm, pertahankan **pnpm**, hapus `package-lock.json` dari repo (`git rm package-lock.json`, commit).
2. Tambahkan field eksplisit di `package.json` supaya tidak ambigu lagi ke depannya:
   ```json
   "packageManager": "pnpm@9.0.0"
   ```
   (sesuaikan angka versi dengan versi pnpm yang benar-benar dipakai, cek `pnpm -v`).
3. Kalau project di Vercel Dashboard punya setting "Install Command" manual yang masih menyebut `npm install`, ubah jadi `pnpm install` (Settings → General → Build & Development Settings), atau kosongkan supaya Vercel auto-detect dari lockfile yang tersisa.
...
1. State di-inisialisasi sebagai angka `useState(0)` (number), bukan string kosong `useState("")` — begitu di-render ke `value={phone}`, React menampilkan `"0"` sebagai starting value.
2. `onChange` handler salah menggabungkan nilai lama + baru (`setPhone(prev => prev + e.target.value)`) alih-alih mengganti penuh (`setPhone(e.target.value)`).
...
### D.4 `/mood-check-in` Diperluas Jadi 15 Pertanyaan

> Bukan lagi cuma "pilih mood + level stres" — `/mood-check-in` harian diperluas jadi kuisioner ringan 15 pertanyaan, dirancang supaya tiap dimensi bisa langsung dipetakan ke 3 aspek Zyba Score (D.2). Ini juga otomatis bikin `calculateMentalScore`/`calculateFisikScore`/`calculateSosialScore` di D.2 punya data yang jauh lebih akurat, bukan cuma tebakan dari mood+aktivitas terpisah.

**15 pertanyaan (dikelompokkan per aspek, urutan tampil boleh diacak/dicampur biar tidak terasa seperti formulir kaku)**:

| # | Pertanyaan | Tipe Jawaban | Aspek |
|---|---|---|---|
| 1 | Bagaimana mood kamu secara keseluruhan hari ini? | 5-level mood selector (sudah ada) | Mental |
| 2 | Seberapa tinggi level stres kamu hari ini? | Skala 1-5 | Mental |
| 3 | Seberapa cemas perasaanmu hari ini? | Skala 1-5 | Mental |
| 4 | Seberapa puas kamu dengan hari ini secara keseluruhan? | Skala 1-5 | Mental |
| 5 | Bagaimana kualitas tidurmu semalam? | Skala 1-5 | Fisik |
| 6 | Berapa jam kamu tidur semalam? | Angka (jam) | Fisik |
| 7 | Seberapa berenergi kamu hari ini? | Skala 1-5 | Fisik |
| 8 | Apakah kamu makan dengan teratur hari ini? | Ya/Tidak | Fisik |
| 9 | Apakah kamu melakukan aktivitas fisik hari ini? | Ya/Tidak | Fisik |
| 10 | Seberapa terhubung kamu dengan orang lain hari ini? | Skala 1-5 | Sosial |
| 11 | Apakah kamu merasa didukung oleh orang sekitarmu hari ini? | Ya/Tidak | Sosial |
| 12 | Apakah kamu berinteraksi dengan komunitas/teman hari ini? | Ya/Tidak | Sosial |
| 13 | Seberapa produktif kamu merasa hari ini? | Skala 1-5 | Mental |
| 14 | Apakah kamu meluangkan waktu untuk diri sendiri hari ini? | Ya/Tidak | Mental |
| 15 | Ada hal positif apa yang terjadi hari ini? (opsional) | Teks bebas | — (gratitude journal, tidak dihitung ke skor) |

Ditambah **satu kolom refleksi/catatan bebas** di akhir (opsional, sama seperti sebelumnya) — ini yang tetap wajib dilewatkan ke `detectRisk()` (Bagian 8/20) sebelum disimpan.

**UI**: ikuti pola multi-step yang sudah ada di `/assessment` (satu/beberapa pertanyaan per layar, progress bar, bisa next/back) — **jangan** taruh 15 pertanyaan di satu layar penuh sekaligus, itu akan terasa berat. Terapkan juga selected-state dari D.1 di semua pilihan skala/ya-tidak ini.

**Update skema `MoodEntry` (Account DB)**:
...
model MoodEntry {
  ...
  mood                MoodLevel  // Q1
  stressLevel         Int?       // Q2, 1-5
  anxietyLevel        Int?       // Q3, 1-5
  satisfactionLevel   Int?       // Q4, 1-5
  sleepQuality        Int?       // Q5, 1-5
  sleepHours          Float?     // Q6
  energyLevel         Int?       // Q7, 1-5
  ateRegularly        Boolean?   // Q8
  didPhysicalActivity Boolean?   // Q9
  socialConnection    Int?       // Q10, 1-5
  feltSupported       Boolean?   // Q11
  interactedCommunity Boolean?   // Q12
  productivityLevel   Int?       // Q13, 1-5
  hadSelfCareTime     Boolean?   // Q14
  gratitudeNote       String?    // Q15, opsional

  note      String?   // refleksi bebas, tetap ada, tetap dicek detectRisk()
  createdAt DateTime  @default(now())
...
> Field lama (`sleepRating`, `energyTags` dari merge Bagian 8.5.1) **digantikan** oleh field baru yang lebih terstruktur ini — kalau ada data lama dengan nama field itu, sesuaikan migrasinya, jangan biarkan dua set field yang tumpang tindih.

**Update formula scoring (D.2)** — sekarang bisa langsung baca field terstruktur, bukan tebakan:

```ts
function calculateMentalScore(entries: MoodEntry[]): number {
  if (entries.length === 0) return 50; // netral kalau belum ada data
  const avg = (key: keyof MoodEntry) => {
    const vals = entries.map((e) => e[key]).filter((v): v is number => typeof v === "number");
    return vals.length ? vals.reduce((a, b) => a + b, 0) / vals.length : 3; // default netral 3/5
  };
  const moodScore = avg("stressLevel" as any) ? (6 - avg("stressLevel" as any)) * 20 : 50; // stres rendah = skor tinggi
  const anxietyScore = (6 - avg("anxietyLevel" as any)) * 20;
  const satisfactionScore = avg("satisfactionLevel" as any) * 20;
  const productivityScore = avg("productivityLevel" as any) * 20;
  return Math.round((moodScore + anxietyScore + satisfactionScore + productivityScore) / 4);
}

function calculateFisikScoreFromMood(entries: MoodEntry[]): number {
  if (entries.length === 0) return 50;
  const sleepScore = avgField(entries, "sleepQuality") * 20;
  const energyScore = avgField(entries, "energyLevel") * 20;
  const activityRate = entries.filter((e) => e.didPhysicalActivity).length / entries.length * 100;
  return Math.round((sleepScore + energyScore + activityRate) / 3);
}

function calculateSosialScoreFromMood(entries: MoodEntry[]): number {
  if (entries.length === 0) return 50;
  const connectionScore = avgField(entries, "socialConnection") * 20;
  const supportRate = entries.filter((e) => e.feltSupported).length / entries.length * 100;
  return Math.round((connectionScore + supportRate) / 2);
}
```

`calculateFisikScore` di D.2 sekarang jadi **gabungan** data `ActivityLog` (Smart Activity Planner) **dan** field fisik dari `MoodEntry` di atas — begitu juga `calculateSosialScore` gabungan `CommunityPost/Comment` **dan** field sosial dari `MoodEntry`. Ini bikin skor lebih representatif karena dari 2 sumber data per aspek, bukan cuma 1.

---

## Lampiran E — Migrasi Penuh ke Self-Hosted (zyba.my.id)

> **Keputusan arsitektur**: pindah dari Vercel + Neon ke hosting sendiri di domain `zyba.my.id` (panel Webuzo, mirip cPanel) — **app dan database dua-duanya pindah**, bukan cuma salah satu.

### E.0 Bagian yang Jadi Superseded (Sudah Tidak Berlaku)

Supaya tidak bingung baca dokumen ini ke depannya, tandai eksplisit: **Lampiran B** (debugging deployment Vercel — `postinstall`, lockfile, `binaryTargets` rhel-openssl) sudah **tidak relevan lagi** kalau app tidak lagi di-deploy ke Vercel. Simpan sebagai catatan sejarah/pembelajaran (terutama soal `binaryTargets` Prisma, itu prinsipnya tetap berlaku di server manapun), tapi jangan diikuti langkah "trigger redeploy Vercel"-nya.

Bagian **20.1** (rekomendasi Vercel Blob untuk storage) juga **digantikan** — lihat E.3.

### E.1 Migrasi Database: Neon → PostgreSQL Self-Hosted

Kabar baik: panel `zyba.my.id` punya **"Manage PostgreSQL"**, bukan cuma MySQL — jadi ini migrasi Postgres-ke-Postgres, bukan konversi antar-mesin database. Struktur 3 database (Bagian 17) **tetap dipertahankan**, cuma host-nya yang beda.

**Langkah**:
1. Di panel Webuzo → **Manage PostgreSQL**, buat 3 database baru: `zyba_account`, `zyba_companion`, `zyba_community` (nama bebas, konsisten aja).
2. Buat user PostgreSQL + kasih akses ke masing-masing database (biasanya ada di halaman yang sama, mirip "Add Database User" versi MySQL).
3. **Export data dari Neon** (kalau sudah ada data yang mau dipertahankan — kalau masih tahap awal/demo dan datanya masih dummy/sedikit, boleh skip ke langkah 5 dan mulai dari `db:push` langsung):
   ```bash
   pg_dump "postgresql://<user>:<pass>@<neon-direct-host>/account?sslmode=require" > account_backup.sql
   pg_dump "postgresql://<user>:<pass>@<neon-direct-host>/companion?sslmode=require" > companion_backup.sql
   pg_dump "postgresql://<user>:<pass>@<neon-direct-host>/community?sslmode=require" > community_backup.sql
   ```
   (pakai `DIRECT_URL_*`, bukan yang pooled, untuk `pg_dump`.)
4. **Import ke server baru** (lewat SSH Terminal yang tersedia di panel):
   ```bash
   psql "postgresql://<user>:<pass>@localhost/zyba_account" < account_backup.sql
   psql "postgresql://<user>:<pass>@localhost/zyba_companion" < companion_backup.sql
   psql "postgresql://<user>:<pass>@localhost/zyba_community" < community_backup.sql
   ```
5. **Update environment variables** — karena server sendiri biasanya tidak punya connection pooler built-in seperti Neon (PgBouncer), `DATABASE_URL_*` dan `DIRECT_URL_*` sekarang **boleh sama persis** (tidak perlu bedakan pooled vs direct):
   ```
   DATABASE_URL_ACCOUNT="postgresql://<user>:<pass>@localhost:5432/zyba_account"
   DIRECT_URL_ACCOUNT="postgresql://<user>:<pass>@localhost:5432/zyba_account"
   # (pola sama untuk COMPANION dan COMMUNITY)
   ```
   > Kalau nanti traffic naik signifikan dan koneksi mulai jadi bottleneck, connection pooler (PgBouncer) bisa ditambah manual lewat SSH — tapi untuk skala kompetisi/demo, koneksi langsung sudah cukup.
6. Jalankan `db:generate:all` (Bagian 17.6) di server baru supaya Prisma Client ter-generate cocok dengan environment Linux server ini (prinsip `binaryTargets` dari Lampiran B tetap relevan — pastikan generate dilakukan **di server yang sama** tempat app jalan, bukan di-copy dari mesin lokal Windows).

### E.2 Migrasi Hosting App: Vercel → Node.js di Webuzo

1. Panel punya **Applications → Add Applications → node** (Node.js 21 tersedia) — gunakan ini untuk setup aplikasi Next.js.
2. **Deploy kode**: panel punya **GitTM Version Control** — paling praktis clone langsung dari `github.com/syauqiazka/zyba_` lewat fitur ini atau lewat SSH (`git clone` manual), daripada upload file satu-satu.
3. Di dalam SSH Terminal panel:
   ```bash
   cd ~/zyba_  # atau lokasi app di-clone
   pnpm install
   pnpm run build
   ```
4. **Environment variables**: buat file `.env` langsung di server (lewat File Manager atau SSH `nano .env`) — isi semua variable dari `.env.example` (Bagian 19.3) dengan nilai production, termasuk `DATABASE_URL_*` yang baru dari E.1. **Jangan** commit file `.env` ini ke git.
5. **Menjalankan app**: Next.js production run pakai `next start` (bukan `next dev`). Setup Node App di panel biasanya minta "Application Startup File" — kalau panel tidak langsung mendukung `next start` sebagai startup command, buat file `server.js` sederhana sebagai entry point:
   ```js
   const { execSync } = require("child_process");
   execSync("next start -p " + (process.env.PORT || 3000), { stdio: "inherit" });
   ```
   Atau cek dokumentasi Webuzo/panel untuk cara paling native menjalankan `next start` — beberapa panel modern sudah punya preset khusus Next.js.
6. **Domain & SSL**: arahkan domain `zyba.my.id` ke aplikasi Node ini (biasanya otomatis kalau app di-setup di document root domain utama). Aktifkan **Automatic SSL** di panel supaya `https://zyba.my.id` jalan dengan HTTPS — **wajib** untuk Midtrans (Bagian 26) dan NextAuth.
7. **Proses tetap hidup**: pastikan Node App manager di panel menjaga proses tetap jalan di background (restart otomatis kalau crash) — biasanya fitur bawaan "Setup Node.js App", tidak perlu setup PM2 manual kecuali panel tidak menyediakan ini.

### E.3 Migrasi Storage: Vercel Blob → Filesystem Server

Karena sekarang punya server sendiri dengan disk (bukan lagi serverless Vercel), **tidak perlu** layanan blob storage eksternal — simpan file media (Bagian 20) langsung di filesystem server:

- Simpan upload (gambar/audio) di folder `public/uploads/` di dalam project, Next.js otomatis serve ini sebagai static file.
- `attachmentUrl`/`imageUrl`/`audioUrl` di database cukup diisi path relatif (mis. `/uploads/xxxx.jpg`), bukan URL Vercel Blob lagi.
- **Ganti** kode upload flow (Bagian 20.5) dari client-upload-ke-Vercel-Blob jadi upload biasa ke API route yang nulis file ke `public/uploads/` di server (pakai `formidable` atau `multer` di API route Next.js untuk handle `multipart/form-data`).
- **Backup**: karena file sekarang di satu server (bukan cloud storage terdistribusi), pastikan fitur **Backups & Restore** (Backuply, sudah ada di panel) dijadwalkan rutin — ini pengganti redundancy yang tadinya didapat gratis dari Vercel Blob.

### E.4 Konfigurasi Lain yang Perlu Diupdate

- `NEXTAUTH_URL="https://zyba.my.id"` (bukan lagi domain `.vercel.app`).
- Webhook Midtrans (Bagian 26) — update URL notification di dashboard Midtrans jadi `https://zyba.my.id/api/billing/webhook`.
- Google OAuth callback URL (Bagian 8.6) — update juga di Google Cloud Console jadi `https://zyba.my.id/api/auth/google/callback`.

### E.5 Checklist Migrasi

- [ ] 3 database PostgreSQL baru dibuat di panel, data (kalau ada) berhasil di-restore dari Neon.
- [ ] `db:generate:all` dijalankan **di server**, bukan di-copy dari mesin lokal.
- [ ] App ter-deploy via Git/SSH, `pnpm install` + `pnpm run build` sukses tanpa error.
- [ ] Domain `zyba.my.id` mengarah ke app, SSL aktif (cek gembok hijau di browser).
- [ ] Semua env var di server sudah lengkap sesuai `.env.example` terbaru, dengan `DATABASE_URL_*` yang baru.
- [ ] Upload gambar/audio tersimpan di `public/uploads/`, bisa diakses via URL, bukan lagi manggil Vercel Blob API.
- [ ] Webhook Midtrans & callback Google OAuth sudah diupdate ke domain baru.
- [ ] Login, kirim pesan Companion, dan posting Community dites langsung di `https://zyba.my.id` — bukan cuma dites lokal.
- [ ] Backup terjadwal (Backuply) sudah diaktifkan.

### E.6 Fix `next.config.js` — Sekali Diperbaiki, Tidak Perlu Diubah Lagi

> **Temuan review kode**: `next.config.js` yang sudah di-upload punya `experimental.serverActions.allowedOrigins: ["localhost:3000"]` — **hardcoded**. Ini akan memblokir Server Actions begitu diakses dari `zyba.my.id` di production (origin request-nya beda dari yang di-whitelist). `server.js` (custom entry point yang sudah dibuat) sudah oke — port 3000 & hostname 127.0.0.1 itu wajar untuk setup reverse-proxy Node App di panel seperti ini.

**Perbaikan** (satu kali, setelah ini `next.config.js` tidak perlu diedit lagi untuk server manapun):
...
Lalu tambahkan di `.env` **di server** (bukan di kode, jadi tidak ikut ter-commit/perlu diubah lagi tiap deploy):
```
ALLOWED_ORIGINS="zyba.my.id,www.zyba.my.id"
```

### E.7 Workflow "Pull-Only" — Realitanya Butuh 3 Langkah, Bukan Cuma `git pull`

Supaya jelas: `.env` sudah aman (di-gitignore, Bagian 8.5.2 — `git pull` **tidak akan** menimpa file ini). Tapi `git pull` sendiri **cuma update file kode**, tidak otomatis install dependency baru / build ulang / restart proses. Supaya workflow-nya konsisten "tinggal jalanin satu perintah" tiap ada update, buat script `deploy.sh` di root project:

```bash
#!/bin/bash
# deploy.sh — jalankan ini di SSH Terminal panel tiap ada update dari GitHub
set -e
git pull origin main
pnpm install
pnpm run db:generate:all
pnpm run build
echo "Build selesai. Restart Node App lewat panel Webuzo (Applications > List Applications > Restart)."
```

Jalankan dengan `bash deploy.sh` tiap update. **Satu langkah manual yang tetap perlu** (tidak bisa full-otomatis dari script ini): klik tombol **Restart** di panel Webuzo (Applications → List Applications) setelah build selesai — proses Node.js yang sedang jalan tidak otomatis reload kode baru tanpa di-restart. Kalau panel Webuzo punya fitur "restart otomatis saat file berubah" (cek dokumentasi Node App-nya), langkah manual ini bisa dihilangkan.
