...
export default function FollowersPage() {
  ...
  const userId = searchParams?.get("userId") || "self";
  const [followers, setFollowers] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (userId === "self") {
      fetch("/api/account/user/me")
        .then((r) => (r.ok ? r.json() : { user: {} }))
        .then((data) => {
          const uid = data.user?.id;
          if (uid) return fetch(`/api/community/users/${uid}/followers`);
    ...
    <div className="flex-1 min-w-0 h-full overflow-y-auto">
      <div className="w-full max-w-[620px] mx-auto py-4 px-2 space-y-3">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="font-bold text-lg text-brown-900">Pengikut</h1>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-8 text-center text-xs text-brown-700/60">
            <p className="font-bold text-sm text-brown-900 mb-1">Belum ada pengikut</p>
            <p className="mb-3">Orang yang mengikutimu akan muncul di sini.</p>
