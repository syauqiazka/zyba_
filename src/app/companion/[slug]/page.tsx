 ...
 * Renders the appropriate view based on activeSection (derived from URL in CompanionContext).
 * Supported slugs: projects, artifacts, code, customize
 */
export default function CompanionSlugPage() {
  const {
    activeSection,
    chatMode,
    setChatMode,
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-7 max-w-sm w-full shadow-2xl border border-brown-900/10 flex flex-col gap-5">
            <div className="flex items-center justify-between">
              <h3 className="font-display font-extrabold text-base text-brown-900">Pengaturan Zyba Companion</h3>
      ...
      {showProModal && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-orange-100 rounded-3xl p-8 max-w-md w-full shadow-2xl border border-orange-500/20 flex flex-col items-center text-center gap-5">
            <div className="w-14 h-14 rounded-2xl bg-orange-500 text-white font-bold flex items-center justify-center text-2xl shadow-md">⚡</div>
            <div>
              <h3 className="font-display font-extrabold text-xl text-brown-900">Kuota Chat Harian Habis</h3>
            ...
            <button type="button" onClick={() => setShowProModal(false)} className="text-xs font-semibold text-brown-700 hover:text-brown-900 hover:underline">
              Nanti saja
            </button>
          </div>
        </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-xl border border-brown-900/10 flex flex-col gap-4">
            <h3 className="font-display font-extrabold text-base text-brown-900">Hapus Percakapan Ini?</h3>
            <p className="text-xs text-brown-700">Semua riwayat obrolan dalam topik ini akan dihapus secara permanen.</p>
