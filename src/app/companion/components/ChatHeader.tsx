    ...
    <header className="px-3 sm:px-6 py-2.5 sm:py-3 border-b border-brown-900/10 flex items-center gap-2 justify-between bg-white/95 sticky top-0 z-10">
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
        ...
        >
          ...
          {/* Online Indicator */}
          <div
            className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-green-500 border-2 border-white"
            title="Online"
          />
        </button>
        ...
        <div className="flex flex-col min-w-0 flex-1">
          <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
            ...
            <span className="hidden md:flex items-center gap-1 text-[11px] text-green-500 font-semibold shrink-0">
              <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
              Online
            </span>
          </div>
          {/* Desktop/Tablet description + model */}
          <span className="hidden sm:block text-[10px] text-brown-700 truncate">
            {persona.description} • {selectedModel}
          </span>
          {/* Mobile concise line */}
          <span className="sm:hidden text-[10px] text-brown-700/80 truncate flex items-center gap-1">
      ...
      <div className="flex items-center gap-1 sm:gap-2 shrink-0">
        {/* Dynamic Emotion Chip — hidden on mobile, visible md+ */}
        <span
          className={`hidden md:flex rounded-pill ${moodColor}/10 text-xs font-semibold px-2.5 py-1 items-center gap-1`}
        ...
        {setIsTTSEnabled && (
          ...
          >
            {isTTSEnabled ? <Volume2 size={15} aria-hidden /> : <VolumeX size={15} aria-hidden />}
          </button>
        )}
        ...
        <button
          ...
          className="p-1.5 sm:p-2 rounded-xl text-brown-700 hover:bg-cream hover:text-brown-900 transition-colors"
        ...
        <button
          ...
          className="p-1.5 sm:p-2 rounded-xl text-brown-700 hover:bg-orange-100 hover:text-danger transition-colors"
