...
interface SettingsTogglesProps {
  ...
  onPersonaChange: (v: PersonaId) => void;
    ...
    <div className="lg:col-span-5 flex flex-col gap-6">
      ...
      <div className="glass-card rounded-3xl p-6 border border-brown-900/10 flex flex-col gap-4 bg-white">
        <h3 className="font-display text-base font-bold text-brown-900 border-b border-brown-900/10 pb-2">
          2. Karakter Zyba Companion
        </h3>
        <p className="text-xs text-brown-700">Pilih karakter yang menemanimu — bisa diganti kapan saja.</p>
        <PersonaPicker selected={selectedPersona} onChange={onPersonaChange} />
      </div>

      {/* Notifikasi — Bagian 24.1 */}
      <div className="glass-card rounded-3xl p-6 border border-brown-900/10 flex flex-col gap-4 bg-white">
        <h3 className="font-display text-base font-bold text-brown-900 border-b border-brown-900/10 pb-2">
        ...
        <div className="flex flex-col gap-3 text-xs text-brown-900">
          <label className="flex items-center justify-between cursor-pointer">
            ...
            <input type="checkbox" checked={fingerprintEnabled} onChange={(e) => onFingerprintChange(e.target.checked)} className="accent-green-500" />
          </label>
          <label className="flex items-center justify-between cursor-pointer">
            <span>Notifikasi Chat Companion</span>
            <input type="checkbox" checked={notifChatbot} onChange={(e) => onNotifChatbotChange(e.target.checked)} className="accent-green-500" />
          </label>
          <label className="flex items-center justify-between cursor-pointer">
            <span>Pengingat Mood Check-In & Tidur</span>
            <input type="checkbox" checked={notifWellness} onChange={(e) => onNotifWellnessChange(e.target.checked)} className="accent-green-500" />
          </label>
          <label className="flex items-center justify-between cursor-pointer">
            <span>Notifikasi Aktivitas Komunitas</span>
            <input type="checkbox" checked={notifCommunity} onChange={(e) => onNotifCommunityChange(e.target.checked)} className="accent-green-500" />
          </label>
        </div>
      </div>
    </div>
